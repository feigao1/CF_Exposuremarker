...
log_RR.formula = expression( u_a - u_0 )
#### First Derivatives ####
log_RR.s_u_a = D(log_RR.formula,'u_a')
log_RR.s_u_0 = D(log_RR.formula,'u_0')
...
fit.counterfactual.inc.EM<-function(ext.cohort, n.active, case.HIV.active, case.RGC.active, alpha=0.05, n.active.RGC = NULL)
{
  ...
  log_RR.est.EM <- log(trial.inc/pred_HIV_inc_trial)
  ...
  gradient.log_RR <- matrix(c(eval(log_RR.s_u_a),eval(log_RR.s_u_0)),2,1)
  
  se.log_RR.est.EM <- sqrt(t(gradient.log_RR) %*% 
                         diag(c(
                           (1 - trial.inc) / n.active.HIV / trial.inc ,
                           se.pred.u^2))  %*% 
                           gradient.log_RR)
  
  log_RR.lwr.EM <- log_RR.est.EM - qnorm(1-alpha/2) * se.log_RR.est.EM 
  log_RR.upr.EM <- log_RR.est.EM + qnorm(1-alpha/2) * se.log_RR.est.EM 
  
  # PE.lwr.EM <- PE.est.EM - qnorm(1-alpha/2) * se.PE.est.EM 
  # PE.upr.EM <- PE.est.EM + qnorm(1-alpha/2) * se.PE.est.EM 
  PE.lwr.EM <- 1 - exp(log_RR.upr.EM)  
  PE.upr.EM <- 1 - exp(log_RR.lwr.EM)
