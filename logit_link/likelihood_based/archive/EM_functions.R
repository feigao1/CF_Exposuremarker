...
sim.incidence<-function(n,logit.mu,logit.sigma, cohort.sizes = c(200,5000))
{
...
sim.incidence.rho_m<-function(n,logit.mu,logit.sigma, cohort.sizes = c(200,5000), rho_m.range = c(.4,.5))
{
  inc.rand<-expit(mvrnorm(n,mu=logit.mu,Sigma=logit.sigma))
  #apply(inc.rand,2,range)
  #apply(inc.rand,2,quantile,c(.1,.9))
...
fit.counterfactual.inc.EM<-function(ext.cohort, n.active, case.HIV.active, case.RGC.active, alpha=0.05, n.active.RGC = NULL)
{
  ...
  pred_HIV_inc_EM <- expit(EM.alpha + EM.beta * logit(case.RGC.active/n.active) )
  ...
  temp.gradident <- c(1, logit(case.RGC.active/n.active))
  temp.se <- sqrt(t(temp.gradident) %*% matrix(c(var.a.coef,cov.a.b.coef,cov.a.b.coef,var.b.coef),2,2) %*% temp.gradident)
  
  pred_HIV_inc_EM.lwr <- expit(EM.alpha + EM.beta * logit(case.RGC.active/n.active) - qnorm(1-alpha/2) * temp.se)
  pred_HIV_inc_EM.upr <- expit(EM.alpha + EM.beta * logit(case.RGC.active/n.active) + qnorm(1-alpha/2) * temp.se)
  ...
  obs.v<-logit(case.RGC.active/n.active) 
  se.v<-sqrt(1/(n.active * expit(obs.v) * (1-expit(obs.v))))
  ...
  ##########################################
  trial.inc <- case.HIV.active/n.active
  ...
  se.PE.est.EM <- sqrt(t(gradient.VE) %*% 
                         diag(c(
                           1/ n.active / trial.inc / (1 - trial.inc),
                           se.pred.u^2))  %*% 
                         gradient.VE)
